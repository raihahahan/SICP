...
(define (install-real-package)
  ...
  (define (negate x)
    (* -1 x))
  (define (expt-real a b)
    (expt a b))
  (define (reduce-real n d)
    (let ((g (gcd n d)))
      (list (/ n g) (/ d g))))
...
(define (install-rational-package)
  ...
  (define (make-rat n d)
    (cond ((=zero? n) 0)
          ((and (not (pair? n)) (not (pair? d))) (cons (car n d) (cadr n d)))
          (else
           (cons (attach-tag (type-tag n) (car (contents (reduce n d)))) (cadr (contents (reduce n d)))))))
  
  (define (expt-rat a b)
    (make-rat (expt (numer a) b)
              (expt (denom a) b)))
  ...
  (put 'exponent '(rational integer)
       (lambda (x y) (tag (expt-rat x y))))
...
(define (install-polynomial-package)
  ...
  (define (pseudoremainder-terms a b)
    (let ((O1 (order (first-term a)))
      ...
      (let ((int-power (+ 1 (- O1 O2))))
        (let ((int-factor (exponent c int-power)))
          (let ((new-dividend (mul-terms (list 'sparse (make-term 0 int-factor)) a)))
            (cadr (div-terms new-dividend b)))))))

  (define (div-by-common-factor L)
    (define (iter largest L)
      (cond ((empty-termlist? L) largest)
            ((>= (coeff (first-term L)) largest)
             (iter (coeff (first-term L)) (rest-terms L)))
            (else
             (iter largest (rest-terms L)))))
    (let ((largest-coeff (iter 0 L)))
      (div-terms L (list 'sparse (make-term 0 largest-coeff)))))
    
  
  (define (gcd-terms a b)
    (if (empty-termlist? b)
        (div-by-common-factor a)
...
(define (exponent term expt) (apply-generic 'exponent term expt))
...
(define P1 (make-polynomial 'x (list 'sparse (make-term 1 1) (make-term 0 1))))
(define P2 (make-polynomial 'x (list 'sparse (make-term 3 1) (make-term 0 -1))))
(define P3 (make-polynomial 'x (list 'sparse (make-term 1 1))))
(define P4 (make-polynomial 'x (list 'sparse (make-term 2 1) (make-term 0 -1))))

(define q1 (make-polynomial 'x (list 'dense 1 -2 1)))
(define q2 (make-polynomial 'x (list 'sparse (make-term 2 11) (make-term 0 7))))
(define q3 (make-polynomial 'x (list 'sparse (make-term 2 1) (make-term 0 1))))
(define q4 (make-polynomial 'x (list 'sparse (make-term 3 1) (make-term 0 1))))

(define Q1 (mul q1 q2))
(define Q2 (mul q1 q3))
;(trace apply-generic)

;(define rf4 (make-rational P1 P2))
;(define rf2 (make-rational P3 P4))

(define rf1 (make-rational Q1 Q2))
(define rf2 (make-rational q1 q2))
(define rf3 (make-rational q3 q4))

(define s1 (make-polynomial 'x (list 'dense 1 2 0)))
(define s2 (make-polynomial 'x (list 'dense 1 0 0 -1)))
(define s3 (make-polynomial 'x (list 'dense 1 0)))
(define s4 (make-polynomial 'x (list 'dense 1 0 -1)))
(define s5 (make-polynomial 'x (list 'dense 1 0)))

;(define t1 (make-rational s1 s2))
;(define t2 (make-rational s3 s4))
(add rf2 rf3)
;(trace apply-generic)
;(make-rational s1 s5)
...
;; OUTPUT 
;'(rational (polynomial x sparse (term 5 1) (term 4 9) (term 3 1) (term 2 19) (term 1 -2) (term 0 8)) polynomial x sparse (term 5 11) (term 3 7) (term 2 11) (term 0 7))
