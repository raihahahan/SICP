...
(define (permutations s)
  (if (null? s) ; if empty set,
      (list nil) ; return the empty set
      (flatmap (lambda (x) ; for each x in the given sequence s
                 (map (lambda (p) (cons x p)) ; for each p in the given sequence s-x,
                      (permutations (remove x s)))) ; adjoin x to the permutation p
               s)))
